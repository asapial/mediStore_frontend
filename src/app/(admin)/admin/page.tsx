...
export default function AdminDashboardPage() {
  ...
  const fetchStats = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${process.env.backendBaseUrl}/api/admin/stats`, {
        method: "GET",
        credentials: "include",
      });
      ...
      console.error(err);
      toast.error(err instanceof Error ? err.message : "Error fetching admin stats");
    } finally {
      setLoading(false);
    }
  };
  ...
  const reviewData = [{ name: "Rating", value: stats.reviews.averageRating }];

  const cardClass =
    "p-6 rounded-2xl shadow-lg bg-gradient-to-r from-emerald-50 to-white dark:from-slate-900 dark:to-slate-800 hover:shadow-2xl hover:scale-[1.03] transition-transform";
